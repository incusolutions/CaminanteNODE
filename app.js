...
app.get('/controller', function(request, response) {
  response.sendfile('controller.html');
});
...
io.sockets.on('connection', function (socket) {
    ...
    socket.on('dia', function( session_id ) {
        io.sockets.socket(session_id).emit('diac');
    });

     socket.on('noche', function( session_id ) {
        io.sockets.socket(session_id).emit('nochec');
    });
